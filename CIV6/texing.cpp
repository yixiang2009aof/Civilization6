...
#include "stdafx.h"
#include "texing.h"
IMPLEMENT_DYNAMIC(Ctexing, CRecordset)

Ctexing::Ctexing(CDatabase* pdb)
	: CRecordset(pdb)
{
	m_ID = 0;
	m_type = L"";
...
// 储为其他格式或使用其他的用户身份验证。
CString Ctexing::GetDefaultConnect()
{
	return _T("DSN=civ6;DBQ=C:\\db.accdb;DriverId=25;FIL=MS Access;MaxBufferSize=2048;PageTimeout=5;UID=admin;");
}

CString Ctexing::GetDefaultSQL()
{
	return _T("[texing]");
}

void Ctexing::DoFieldExchange(CFieldExchange* pFX)
{
...
#ifdef _DEBUG
void Ctexing::AssertValid() const
{
	CRecordset::AssertValid();
}

void Ctexing::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
